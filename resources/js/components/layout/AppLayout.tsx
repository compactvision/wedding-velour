...
function MobileNavigation() {
  ...
  const actionItems = [
    ...(['admin', 'server'].includes(user?.role) ? [{ path: '/server', label: 'Serveur' }] : []),
    ...(['admin', 'door'].includes(user?.role) ? [{ path: '/door', label: 'Porte' }] : []),
  ];
  ...
  return (
    <>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-[88vw] max-w-sm p-0">
                ...
                <nav className="grid gap-2 p-4">
                  {[...visibleNavItems, ...actionItems].map(({ path, label, icon: Icon }: any) => (
                    <Link
                      key={path}
                      href={path}
                      className={cn(
                    ...
                    >
                      {Icon ? <Icon className="h-5 w-5 shrink-0" /> : <MoreHorizontal className="h-5 w-5 shrink-0" />}
                      <span>{label}</span>
                    </Link>
                  ))}
                  <Link
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border/80 bg-card/96 px-2 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 shadow-[0_-12px_35px_rgba(0,0,0,0.08)] backdrop-blur-xl md:hidden">
        <div className="grid grid-cols-5 gap-1">
          {primaryItems.map(({ path, icon: Icon, label }) => (
            ...
            >
              ...
              <span className="max-w-full truncate">{label.replace('Tableau de bord', 'Accueil').replace('Plan de salle', 'Salle')}</span>
            </Link>
          ))}
          <Sheet>
            <SheetTrigger asChild>
            ...
            <SheetContent side="bottom" className="max-h-[82svh] rounded-t-lg pb-[max(env(safe-area-inset-bottom),1rem)]">
              <SheetHeader className="text-left">
                <SheetTitle className="font-display">Plus d'actions</SheetTitle>
              </SheetHeader>
              <div className="mt-4 grid grid-cols-2 gap-2">
                {[...secondaryItems, ...actionItems].map(({ path, label, icon: Icon }: any) => (
                  <Link
                    key={path}
                    href={path}
                    className={cn(
                      "flex min-h-14 items-center gap-3 rounded-md border px-3 text-sm font-medium",
                      isActive(path) ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-foreground"
                    )}
                  >
                    {Icon ? <Icon className="h-5 w-5 shrink-0" /> : <MoreHorizontal className="h-5 w-5 shrink-0" />}
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background font-sans">
      ...
      <main className={cn(
        ...
        <div className="mx-auto w-full max-w-[1600px] px-4 pb-28 pt-5 sm:px-5 md:p-6 lg:p-8">
